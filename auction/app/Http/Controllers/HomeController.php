...
class HomeController extends Controller
{
    ...
    public function pincode(Request $request)
    {
        if($request->has('pincode')){
            $a = \App\Pincode::where('pincode', $request->pincode)->first();
            if($a!=null && $a->status == "1") {
                flash(translate('Order Now, We are available on your pincode...'))->success();
                $request->session()->put('pincode', true);
                $request->session()->put('Opincode', $request->pincode);
            } else {
                flash(translate('Sorry we currently unavailable on this pincode...'))->error();
                $request->session()->put('pincode', false);
                $request->session()->put('Opincode', $request->pincode);
            }
        } else {
            flash(translate('Please Enter valid pincode...'))->error();
            $request->session()->put('pincode', false);
            $request->session()->put('Opincode', $request->pincode);
        }
        return back();
...
//        }
    ...
    public function new_verify(Request $request)
    {
		...
		$user = User::where('new_email_verificiation_code', $verification_code)->first();
            ...
            if($user != null) {
				...
				$response['status'] = 1;
            $response['message'] = translate("Your mail has been Saved.");
