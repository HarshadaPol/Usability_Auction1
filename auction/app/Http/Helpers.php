...
if (!function_exists('filter_products')) {
    function filter_products($products)
    {
        ...
        if (get_setting('vendor_system_activation') == 1) {
            return $products->where('approved', '1')->where('published', '1')->where('auction_product', 1)->orderBy('created_at', 'desc')->where(function ($p) use ($verified_sellers) {
                $p->where('added_by', 'admin')->orWhere(function ($q) use ($verified_sellers) {
                    $q->whereIn('user_id', $verified_sellers);
                });
            });
        } else {
            return $products->where('published', '1')->where('auction_product', 1)->where('added_by', 'admin');
        }
    }
}
...
if (!function_exists('get_cached_products')) {
    ...
    {
        $products = \App\Product::where('published', 1)->where('approved', '1')->where('auction_product', 1);
        $verified_sellers = verified_sellers_id();
        if (get_setting('vendor_system_activation') == 1) {
